...
our integer_arrayref_arrayref::method $escape_time = sub {
    ( my MathPerl::Fractal::Julia $self, my number $real_c, my number $imaginary_c, my integer $x_pixel_count, my integer $y_pixel_count, my integer $iterations_max, my number $x_min, my number $x_max, my number $y_min, my number $y_max, my boolean $color_invert ) = @_;
...
our integer_arrayref_arrayref $julia_escape_time = sub {
    ( my number $real_c, my number $imaginary_c, my integer $x_pixel_count, my integer $y_pixel_count, my integer $iterations_max, my number $x_min, my number $x_max, my number $y_min, my number $y_max, my boolean $color_invert )
        = @_;
...
# START HERE: figure out proper initial values below
# START HERE: figure out proper initial values below
# START HERE: figure out proper initial values below

#            my number $real        = 1.5 * ($x_pixel - $x_pixel_count / 2) / (0.5 * zoom * $x_pixel_count) + moveX;
#            my number $imaginary   = ($y_pixel - $y_pixel_count / 2) / (0.5 * zoom * $y_pixel_count) + moveY;
            my number $real        = 1.5 * $x_min;
            my number $imaginary   = $y_min;
            my integer $i       = 0;
            ...
            if ($color_invert) {
                $julia_set->[$y_pixel]->[$x_pixel] = 255 - ( $i * $color_scaling_factor );    # scale to become color, invert for white background
            }
            else {
                $julia_set->[$y_pixel]->[$x_pixel] = $i * $color_scaling_factor;  # scale to become color, black background
            }
        }
    }
    return $julia_set;
